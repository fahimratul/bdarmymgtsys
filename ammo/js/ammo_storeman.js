...
function checkAmmoValidation(ammoType) {
    const ammoRef = ref(db, 'ammoindex/typesofammo/' + ammoType);
...
function addAmmo() {
    ...
    checkAmmoValidation(ammoType).then((exists) => {
        ...
        update(newAmmoRef, {
            ...
            console.error("Error adding Type of Ammo:", error);
            showNotification("Error adding Type of Ammo", "error", "Add Failed");
        });
    }).catch((error) => {
        console.error("Error checking Type of Ammo:", error);
        showNotification("Error checking Type of Ammo", "error", "Error");
    });
}
