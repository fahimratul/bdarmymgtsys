...
function pendingitems(){
    ...
    const dbRef = ref(db, 'unservicable_storeman/medical/');
    let html='';
...
function removeItemRow(rowId, sectionId, parentSectionId, itemID) {
    ...
    remove(ref(db, `unservicable_storeman/medical/${parentSectionId}/items/${itemID}`));
    ...
    if (itemsContainer.children.length === 0) {
        ...
        remove(ref(db, `unservicable_storeman/medical/${key}`));
...
function processIssueRequest(key) {
    ...
    for (const row of itemRows) {
        ...
        const mainitem = get(ref(db, `medical/main/${itemkey}`));
        mainitem.then((snapshot) => {
            ...
            set(ref(db, `medical/${itemkey}/unsvc/${voucherNumber}`), {
                date: issueDate,
                quantity: quantity,
                reason: reason,

            });
            update(ref(db, `medical/main/${itemkey}`), {
