...
function pendingnewitemdata() {
    let dbRef =ref(db, 'officerapproval/new/medical/');
...
function pendingnewtotalitemdata() {
    let dbRef =ref(db, 'officerapproval/newtotal/medical/');
...
function approveNewtotalItem(key) {
    ...
    update(ref(db, 'medical/main/' + key), {
        ...
        remove(ref(db, 'officerapproval/newtotal/medical/' + key)).then(() => {
...
function approveNewItem(key) {
    ...
    if (!newItem) return;
    set(ref(db, 'medical/main/' + key), newItem).then(() => {
        console.log('New item approved and added to inventory');
        remove(ref(db, 'officerapproval/new/medical/' + key)).then(() => {
    ...
    set(ref(db, 'clo_cc_notification/' + Date.now()), {
        from: 'Medical Store Inventory',
        msg: `New Medical Inventory item "${newItem.name || ''}" has been added by ${sessionStorage.getItem('username')} (BA Number: ${sessionStorage.getItem('baNumber')}).`,
        date: new Date().toLocaleString()
    }).then(() => {
...
function newPendingItemNotification(){
    ...
    onValue(ref(db, 'issuepending/medical/'), (snapshot) => {
        if(snapshot.exists()){
    ...
    onValue(ref(db, 'unservicable_storeman/medical/'), (snapshot) => {
        if(snapshot.exists()){
            let html = fixedNotification.innerHTML;
            const id = Date.now();
            html += `<div class="notification-content" id="pending_${id}">
            <p id="notificationMessage">You have a new <strong>Unserviceable</strong> item From Medical Store.</p>
            <button class="notification-close" onclick="hidefixedNotification('pending_${id}')" aria-label="Close">&times;</button>
...
function loadreturnnotification(){
    ...
    onValue(ref(db, 'notification/medical/'), (snapshot) => {
...
function acknowledgeNotification(key){
    remove(ref(db, `notification/medical/${key}`)).then(() => {
...
editForm?.addEventListener('submit', (e) => {
    ...
    update(ref(db, 'medical/main/' + currentEditKey), updated)
        .then(() => {
        ...
        set(ref(db, 'clo_cc_notification/' + Date.now()), {
            from: 'Medical Store Inventory',
            date: new Date().toLocaleString(),
            msg: `Medical Store Inventory item "${inputs.name.value.trim()}" has been updated by ${sessionStorage.getItem('username')} (BA Number: ${sessionStorage.getItem('baNumber')}).`
        }).then(() => {
            console.log('CLO/CC notified successfully about the update.');
        }).catch((error) => {
            console.error('Error notifying CLO/CC about the update:', error);
...
deleteItemBtn?.addEventListener('click', (e) => {
    ...
    remove(ref(db, 'medical/main/' + currentEditKey))
    .then(() => {
    ...
    remove(ref(db, 'medical/' + currentEditKey)).then(() => {
    ...
    set(ref(db, 'clo_cc_notification/' + Date.now()), {
        from: 'Medical Store Inventory',
        date: new Date().toLocaleString(),
        msg: `Medical Store Inventory item "${inputs.name.value.trim()}" has been deleted by ${sessionStorage.getItem('username')} (BA Number: ${sessionStorage.getItem('baNumber')}).`
    }).then(() => {
...
function printReport(tableRows, title, summaryData) {
    ...
    const printContent = `
        ...
        <body>
            ...
            <div class="footer">
                <p>BANRDB Store Management System - Medical Inventory Report</p>
