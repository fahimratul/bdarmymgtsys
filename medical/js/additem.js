...
function checkInventoryItem(name) {
    ...
    let dbRef = ref(db, 'medical/main/' + newname);
...
function writeInventoryItem(name, data) {
    ...
    checkInventoryItem(newname).then((exists) => {
        if (exists) {
            ...
            if(role === 'medicalnco') {
                set(ref(db, 'officerapproval/new/medical/' + newname),{
                name: name,
                unit: data.unit,
                ...
                set(ref(db, 'medical/main/' + newname),{
                    name: name,
                    unit: data.unit,
                    authorized: data.authorized,
                    total: data.total,
                ...
                set(ref(db, 'clo_cc_notifications/' + Date.now()),{
                    ...
                    msg: `New inventory item "${name}" has been added by Medical Officer.`
                });
            }
            else {
