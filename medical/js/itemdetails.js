...
function loaditemsdetails() {
    ...
    let dbRef = ref(db, `medical/main/`+ itemKey);
...
function loaditemhistory() {
    let dbRef = ref(db, `medical/`+ itemKey + `/history`);
    const loadingOverlay = document.getElementById('loadingOverlay');
    onValue(dbRef, (snapshot) => {
...
function loaditemunsvc() {
    let dbRef = ref(db, `medical/`+ itemKey + `/unsvc`);
    const loadingOverlay = document.getElementById('loadingOverlay');
    onValue(dbRef, (snapshot) => {
        const data = snapshot.val();
        itemunsvccache = data || {};
...
function returnItemToStore(recordKey) {
    ...
    path = `medical/${itemKey}/history/${recordKey}`;
    ...
    loaditemsdetails();
    if(role === 'bknco'){
        const notificationPath = `notification/medical/${Date.now()}`;
...
function markAsServicable(recordKey) {
    ...
    const path = `medical/${itemKey}/unsvc/${recordKey}`;
