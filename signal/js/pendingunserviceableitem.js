...
function removeItemRow(rowId, sectionId, parentSectionId, itemID) {
    ...
    remove(ref(db, `unservicable_storeman/so/${parentSectionId}/items/${itemID}`));
    ...
    if (itemsContainer.children.length === 0) {
        ...
        remove(ref(db, `unservicable_storeman/so/${key}`));
...
function processIssueRequest(key) {
    ...
    for (const row of itemRows) {
        ...
        const mainitem = get(ref(db, `siginventory/main/${itemkey}`));
        mainitem.then((snapshot) => {
            ...
            set(ref(db, `siginventory/${itemkey}/unsvc/${voucherNumber}`), {
                date: issueDate,
                quantity: quantity,
                reason: reason,
