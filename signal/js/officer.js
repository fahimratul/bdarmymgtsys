...
function newPendingItemNotification(){
    ...
    onValue(ref(db, 'issuepending/so/'), (snapshot) => {
        if(snapshot.exists()){
            ...
            <p id="notificationMessage"> You have a new <strong> Pending Issue item </strong>  for issue.</p>
    ...
    onValue(ref(db, 'unservicable_storeman/so/'), (snapshot) => {
        if(snapshot.exists()){
            ...
            const id = Date.now();
            html += `<div class="notification-content" id="pending_${id}">
            <p id="notificationMessage">You have a new <strong>Unserviceable</strong> item.</p>
            <button class="notification-close" onclick="hidefixedNotification('pending_${id}')" aria-label="Close">&times;</button>
...
deleteItemBtn?.addEventListener('click', (e) => {
    ...
    remove(ref(db, 'siginventory/' + currentEditKey)).then(() => {
        console.log('Item deleted successfully');
        loaditemdata();
        showNotification('Inventory item deleted successfully.', 'success', 'Item Deleted');
    }).catch((error) => {
        console.error('Error deleting item:', error);
        showNotification('Error deleting item. Please try again.', 'error', 'Deletion Failed');
    });
     set(ref(db, 'clo_cc_notification/' + Date.now()), {
        msg: `Signal Inventory item "${inputs.name.value.trim()}" has been deleted by ${sessionStorage.getItem('username')} (BA Number: ${sessionStorage.getItem('baNumber')}).`
    }).then(() => {
