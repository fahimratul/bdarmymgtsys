...
function loaditemsdetails() {
    ...
    let dbRef = ref(db, `cimic/main/`+ itemKey);
...
function loaditemhistory() {
    let dbRef = ref(db, `cimic/`+ itemKey + `/history`);
    const loadingOverlay = document.getElementById('loadingOverlay');
    onValue(dbRef, (snapshot) => {
...
function loaditemunsvc() {
    let dbRef = ref(db, `cimic/`+ itemKey + `/unsvc`);
    const loadingOverlay = document.getElementById('loadingOverlay');
    onValue(dbRef, (snapshot) => {
        const data = snapshot.val();
        itemunsvccache = data || {};
...
function returnItemToStore(recordKey) {
    ...
    path = `cimic/${itemKey}/history/${recordKey}`;
...
function markAsServicable(recordKey) {
    ...
    const path = `cimic/${itemKey}/unsvc/${recordKey}`;
