...
function pendingnewitemdata() {
    let dbRef =ref(db, 'officerapproval/new/cimic/');
...
function pendingnewtotalitemdata() {
    let dbRef =ref(db, 'officerapproval/newtotal/cimic/');
...
function approveNewtotalItem(key) {
    ...
    update(ref(db, 'cimic/main/' + key), {
        ...
        remove(ref(db, 'officerapproval/newtotal/cimic/' + key)).then(() => {
    ...
    set(ref(db, 'clo_cc_notification/' + Date.now()), {
        ...
        msg: `Cimic Inventory item "${currentItem.name || ''}" total has been updated by ${sessionStorage.getItem('username')} (BA Number: ${sessionStorage.getItem('baNumber')}).`,
...
function approveNewItem(key) {
    ...
    if (!newItem) return;
    set(ref(db, 'cimic/main/' + key), newItem).then(() => {
        console.log('New item approved and added to inventory');
        remove(ref(db, 'officerapproval/new/cimic/' + key)).then(() => {
...
function newPendingItemNotification(){
    ...
    onValue(ref(db, 'issuepending/cimic/'), (snapshot) => {
        if(snapshot.exists()){
            let html = fixedNotification.innerHTML;
            const id = Date.now();
            html += `<div class="notification-content" id="pending_${id}">
            <p id="notificationMessage"> You have a new <strong> Pending Issue item </strong></p>
    ...
    onValue(ref(db, 'unservicable_storeman/cimic/'), (snapshot) => {
        if(snapshot.exists()){
            let html = fixedNotification.innerHTML;
            const id = Date.now();
            html += `<div class="notification-content" id="pending_${id}">
            <p id="notificationMessage">You have a new <strong>Unserviceable item</strong> .</p>
            <button class="notification-close" onclick="hidefixedNotification('pending_${id}')" aria-label="Close">&times;</button>
...
function loadreturnnotification(){
    ...
    onValue(ref(db, 'notification/cimic/'), (snapshot) => {
...
function acknowledgeNotification(key){
    remove(ref(db, `notification/cimic/${key}`)).then(() => {
...
editForm?.addEventListener('submit', (e) => {
    ...
    update(ref(db, 'cimic/main/' + currentEditKey), updated)
        .then(() => {
...
deleteItemBtn?.addEventListener('click', (e) => {
    ...
    remove(ref(db, 'cimic/main/' + currentEditKey))
    .then(() => {
    ...
    remove(ref(db, 'cimic/' + currentEditKey)).then(() => {
        console.log('Item data removed from main inventory after deletion');
    }).catch((error) => {
        console.error('Error removing item data from main inventory after deletion:', error);
    }   );
    set(ref(db, 'clo_cc_notification/' + Date.now()), {
        from: 'Cimic Store Inventory',
        date: new Date().toLocaleString(),  
        msg: `Cimic Store Inventory item "${inputs.name.value.trim()}" has been deleted by ${sessionStorage.getItem('username')} (BA Number: ${sessionStorage.getItem('baNumber')}).`
    }).then(() => {
...
function printReport(tableRows, title, summaryData) {
    ...
    const printContent = `
        ...
        <body>
            ...
            <div class="footer">
                <p>BANRDB Store Management System - cimic Inventory Report</p>
