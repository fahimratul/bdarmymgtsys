...
function pendingitems(){
    ...
    const dbRef = ref(db, 'unservicable_storeman/cimic/');
    let html='';
...
function removeItemRow(rowId, sectionId, parentSectionId, itemID) {
    ...
    remove(ref(db, `unservicable_storeman/cimic/${parentSectionId}/items/${itemID}`));
    ...
    if (itemsContainer.children.length === 0) {
        ...
        remove(ref(db, `unservicable_storeman/cimic/${key}`));
...
function processIssueRequest(key) {
    ...
    for (const row of itemRows) {
        ...
        const mainitem = get(ref(db, `cimic/main/${itemkey}`));
        mainitem.then((snapshot) => {
            ...
            set(ref(db, `cimic/${itemkey}/unsvc/${voucherNumber}`), {
                date: issueDate,
                quantity: quantity,
                reason: reason,

            });
            update(ref(db, `cimic/main/${itemkey}`), {
