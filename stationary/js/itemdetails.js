...
function loaditemsdetails() {
    ...
    let dbRef = ref(db, `stationary/main/`+ itemKey);
...
function loaditemhistory() {
    let dbRef = ref(db, `stationary/`+ itemKey + `/history`);
    const loadingOverlay = document.getElementById('loadingOverlay');
    onValue(dbRef, (snapshot) => {
...
function loaditemunsvc() {
    let dbRef = ref(db, `stationary/`+ itemKey + `/unsvc`);
    const loadingOverlay = document.getElementById('loadingOverlay');
    onValue(dbRef, (snapshot) => {
        const data = snapshot.val();
        itemunsvccache = data || {};
...
function returnItemToStore(recordKey) {
    ...
    path = `stationary/${itemKey}/history/${recordKey}`;
...
function markAsServicable(recordKey) {
    ...
    const path = `stationary/${itemKey}/unsvc/${recordKey}`;
