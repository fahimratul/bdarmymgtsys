...
function loaditemdata() {
    ...
    let dbRef =ref(db, 'stationary/main/');
...
function pendingnewitemdata() {
    let dbRef =ref(db, 'officerapproval/new/stationary/');
...
function pendingnewtotalitemdata() {
    let dbRef =ref(db, 'officerapproval/newtotal/stationary/');
...
function approveNewtotalItem(key) {
    ...
    update(ref(db, 'stationary/main/' + key), {
        ...
        remove(ref(db, 'officerapproval/newtotal/stationary/' + key)).then(() => {
    ...
    set(ref(db, 'clo_cc_notification/' + Date.now()), {
        ...
        msg: `Stationary Inventory item "${currentItem.name || ''}" total has been updated by ${sessionStorage.getItem('username')} (BA Number: ${sessionStorage.getItem('baNumber')}).`,
...
function approveNewItem(key) {
    ...
    if (!newItem) return;
    set(ref(db, 'stationary/main/' + key), newItem).then(() => {
        console.log('New item approved and added to inventory');
        remove(ref(db, 'officerapproval/new/stationary/' + key)).then(() => {
...
function newPendingItemNotification(){
    ...
    onValue(ref(db, 'issuepending/stationary/'), (snapshot) => {
        if(snapshot.exists()){
    ...
    onValue(ref(db, 'unservicable_storeman/stationary/'), (snapshot) => {
        if(snapshot.exists()){
            let html = fixedNotification.innerHTML;
            const id = Date.now();
            html += `<div class="notification-content" id="pending_${id}">
...
function loadreturnnotification(){
    ...
    onValue(ref(db, 'notification/stationary/'), (snapshot) => {
...
function acknowledgeNotification(key){
    remove(ref(db, `notification/stationary/${key}`)).then(() => {
...
deleteItemBtn?.addEventListener('click', (e) => {
    ...
    remove(ref(db, 'stationary/main/' + currentEditKey))
    .then(() => {
