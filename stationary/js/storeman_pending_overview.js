...
async function loadPendingIssues() {
    try {
        const dbRef = ref(db, 'issuepending/stationary/');
...
window.rejectRequest = function(requestId) {
    if (confirm('Are you sure you want to reject this request?')) {
        const dbRef = ref(db, 'issuepending/stationary/' + requestId);
        remove(dbRef)
            .then(() => {
...
window.markAsDisposed = function(itemId) {
    if (confirm('Are you sure you want to cancel this unserviceable item request?')) {
        const dbRef = ref(db, 'unservicable_storeman/stationary/' + itemId);
        remove(dbRef)
            .then(() => {
