...
function pendingitems(){
    ...
    const dbRef = ref(db, 'unservicable_storeman/stationary/');
    let html='';
                ...
                html+=`
                        <div class="item-row" id="item-pending-${key}-${itemkey}">
                            ...
                            <div class="item-form">
                                ...
                                <div class="form-group">
                                    <label for="itemSelect-${key}-${itemkey}">Held & Serviceable</label>
...
function removeItemRow(rowId, sectionId, parentSectionId, itemID) {
    ...
    remove(ref(db, `unservicable_storeman/stationary/${parentSectionId}/items/${itemID}`));
    ...
    if (itemsContainer.children.length === 0) {
        ...
        remove(ref(db, `unservicable_storeman/stationary/${key}`));
...
function processIssueRequest(key) {
    ...
    for (const row of itemRows) {
        ...
        const mainitem = get(ref(db, `stationary/main/${itemkey}`));
        mainitem.then((snapshot) => {
            ...
            set(ref(db, `stationary/${itemkey}/unsvc/${voucherNumber}`), {
                date: issueDate,
                quantity: quantity,
                reason: reason,

            });
            update(ref(db, `stationary/main/${itemkey}`), {
