...
function pendingitems(){
    ...
    const dbRef = ref(db, 'unservicable_storeman/stationary/'+issueItemkey);
    let html='';
    get(dbRef).then((snapshot) => {
        pendingItemsDataCaches = snapshot.val() || {};
        console.log('Pending Items data loaded:', pendingItemsDataCaches);
        document.getElementById('issuedate').value = pendingItemsDataCaches.issueDate || '';
            ...
            Object.keys(pendingItemsDataCaches.items).forEach((key, index) => {
                ...
                html += `
                    <div class="item-row" id="itemRow-${index}">
                        ...
                        <div class="item-reason">Reason<h3>${item.reason}</h3></div>
                    </div>
                `;
                itemskey[index]=key;
            });
