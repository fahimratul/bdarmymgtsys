...
function approveNewtotalItem(key) {
    ...
    update(ref(db, 'workshop/main/' + key), {
        ...
        remove(ref(db, 'officerapproval/newtotal/workshop/' + key)).then(() => {
    ...
    set(ref(db, 'clo_cc_notification/' + Date.now()), {
        ...
        msg: `Workshop Inventory item "${currentItem.name || ''}" total has been updated by ${sessionStorage.getItem('username')} (BA Number: ${sessionStorage.getItem('baNumber')}).`,
...
function approveNewItem(key) {
    ...
    if (!newItem) return;
    set(ref(db, 'workshop/main/' + key), newItem).then(() => {
        console.log('New item approved and added to inventory');
        remove(ref(db, 'officerapproval/new/workshop/' + key)).then(() => {
    ...
    set(ref(db, 'clo_cc_notification/' + Date.now()), {
        from: 'Workshop Inventory',
        msg: `New Workshop Inventory item "${newItem.name || ''}" has been added by ${sessionStorage.getItem('username')} (BA Number: ${sessionStorage.getItem('baNumber')}).`,
        date: new Date().toLocaleString()
    }).then(() => {
...
editForm?.addEventListener('submit', (e) => {
   ...
   const updated = {
    ...
    update(ref(db, 'workshop/main/' + currentEditKey), updated)
        .then(() => {
        ...
        set(ref(db, 'clo_cc_notification/' + Date.now()), {
            from: 'Workshop Inventory',
            date: new Date().toLocaleString(),
            msg: `Workshop Inventory item "${inputs.name.value.trim()}" has been updated by ${sessionStorage.getItem('username')} (BA Number: ${sessionStorage.getItem('baNumber')}).`
        }).then(() => {
            console.log('CLO/CC notified successfully about the update.');
        }).catch((error) => {
            console.error('Error notifying CLO/CC about the update:', error);
...
deleteItemBtn?.addEventListener('click', (e) => {
    ...
    remove(ref(db, 'workshop/main/' + currentEditKey))
    .then(() => {
    ...
    remove(ref(db, 'workshop/' + currentEditKey)).then(() => {
    ...
    set(ref(db, 'clo_cc_notification/' + Date.now()), {
        from: 'Workshop Inventory',
        date: new Date().toLocaleString(),
        msg: `Workshop Inventory item "${inputs.name.value.trim()}" has been deleted by ${sessionStorage.getItem('username')} (BA Number: ${sessionStorage.getItem('baNumber')}).`
    }).then(() => {
