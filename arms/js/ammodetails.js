...
function deleteAmmo(lotnokey) {
    if (confirm("Are you sure you want to delete this arms lot? This action cannot be undone.")) {
        ...
        remove(ammoRef)
            .then(() => {
                showNotification("Arms lot deleted successfully", "success", "Deleted");
                loadammodata();
            })
            .catch((error) => {
                console.error("Error deleting arms lot:", error);
                showNotification("Error deleting arms lot", "error", "Error");
            });
    }
}
...
function updatelotdetails() {
    ...
    const ammoRef = ref(db, 'armsindex/' + ammokey + '/' + editlotnokey);
