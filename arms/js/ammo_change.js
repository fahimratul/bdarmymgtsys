...
function loadNewArrivals() {
    const dbRef = ref(db, 'officerapproval/typesofarms/');
...
function acceptnew(key){
    const dbRef = ref(db, 'officerapproval/typesofarms/' + key);
    get(dbRef).then((snapshot) => {
        if (snapshot.exists()) {
            const item = snapshot.val();
            const newItemRef = ref(db, 'armsindex/typesofarms/' + key);
            set(newItemRef, item).then(() => {
                remove(dbRef).then(() => {
                    showNotification("Arms type approved successfully", "success", "Success");
                }).catch((error) => {
                    console.error("Error removing pending arms type:", error);
...
function rejectnew(key){
    const dbRef = ref(db, 'officerapproval/typesofarms/' + key);
    remove(dbRef).then(() => {
        showNotification("arms type rejected successfully", "success", "Success");
    }).catch((error) => {
        console.error("Error rejecting arms type:", error);
        showNotification("Error rejecting arms type", "error", "Error");
    });
}

function loadPendingLots() {
    const dbRef = ref(db, 'officerapproval/newarms/');
    ...
    onValue(dbRef, (snapshot) => {
        ...
        const count = Object.keys(data).length;
        for (const key in data) {
            console.log('Processing arms type key:', key);
            const dbref =ref(db, 'armsindex/typesofarms/' + key);
            get(dbref).then((snapshot) => {
                const item = snapshot.val();
                console.log('Processing arms type:', item);
                html += `
                <tr><td colspan=19><strong>Arms Type: ${item.name || 'N/A'} - ${item.type || 'N/A'}</strong></td></tr>`
...
function acceptnewlot(compositeKey){
    const dbRef = ref(db, 'officerapproval/newarms/' + compositeKey);
    get(dbRef).then((snapshot) => {
        if (snapshot.exists()) {
            const item = snapshot.val();
            const newItemRef = ref(db, 'armsindex/' + compositeKey);
            set(newItemRef, item).then(() => {
                remove(dbRef).then(() => {
                    showNotification("Arms lot approved successfully", "success", "Success");
                }).catch((error) => {
                    console.error("Error removing pending arms lot:", error);
                    showNotification("Error removing pending arms lot", "error", "Error");
                });
            }).catch((error) => {
                console.error("Error approving arms lot:", error);
                showNotification("Error approving arms lot", "error", "Error");
            });
        } else {
            showNotification("Arms lot not found", "error", "Error");
        }   
    }).catch((error) => {
        console.error("Error fetching arms lot:", error);
        showNotification("Error fetching arms lot", "error", "Error");
    });
}
...
function rejectnewlot(compositeKey){
    const dbRef = ref(db, 'officerapproval/newarms/' + compositeKey);
    remove(dbRef).then(() => {
        showNotification("Arms lot rejected successfully", "success", "Success");
    }).catch((error) => {
        console.error("Error rejecting arms lot:", error);
        showNotification("Error rejecting arms lot", "error", "Error");
    });
}


function loadLotUpdates(){
    const dbRef = ref(db, 'officerapproval/armsindexupdate/');
...
function updatelots(compositeKey){
    const dbRef = ref(db, 'officerapproval/armsindexupdate/' + compositeKey);
    get(dbRef).then((snapshot) => {
        if (snapshot.exists()) {
            const item = snapshot.val();
            const newItemRef = ref(db, 'armsindex/' + compositeKey);
            set(newItemRef, item).then(() => {
                remove(dbRef).then(() => {
                    showNotification("Arms lot update approved successfully", "success", "Success");
                }).catch((error) => {
                    console.error("Error removing pending arms lot update:", error);
                    showNotification("Error removing pending arms lot update", "error", "Error");
                });
            }).catch((error) => {
                console.error("Error approving arms lot update:", error);
                showNotification("Error approving arms lot update", "error", "Error");
            });
        } else {
            showNotification("Arms lot update not found", "error", "Error");
        }
    }).catch((error) => {
        console.error("Error fetching arms lot update:", error);
        showNotification("Error fetching arms lot update", "error", "Error");
    });
}
...
function rejectlotupdate(compositeKey){
    const dbRef = ref(db, 'officerapproval/armsindexupdate/' + compositeKey);
    remove(dbRef).then(() => {
        showNotification("Arms lot update rejected successfully", "success", "Success");
        updateTotalCount();
    }).catch((error) => {
        console.error("Error rejecting arms lot update:", error);
        showNotification("Error rejecting arms lot update", "error", "Error");
    });
}
