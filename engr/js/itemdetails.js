...
function loaditemsdetails() {
    ...
    let dbRef = ref(db, `engrinventory/main/`+ itemKey);
...
function loaditemhistory() {
    let dbRef = ref(db, `engrinventory/`+ itemKey + `/history`);
    const loadingOverlay = document.getElementById('loadingOverlay');
    get(dbRef).then((snapshot) => {
...
function loaditemunsvc() {
    let dbRef = ref(db, `engrinventory/`+ itemKey + `/unsvc`);
    const loadingOverlay = document.getElementById('loadingOverlay');
    get(dbRef).then((snapshot) => {
        const data = snapshot.val();
        itemunsvccache = data || {};
...
function returnItemToStore(recordKey) {
    ...
    let path;
    path = `engrinventory/main/`+ itemKey;  
    const updates = {};
    ...
    path = `engrinventory/${itemKey}/history/${recordKey}`;
...
function markAsServicable(recordKey) {
    ...
    const path = `engrinventory/${itemKey}/unsvc/${recordKey}`;
