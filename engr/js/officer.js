...
deleteItemBtn?.addEventListener('click', (e) => {
    ...
    remove(ref(db, 'engrinventory/' + currentEditKey)).then(() => {
     ...
     set(ref(db, 'clo_cc_notification/' + Date.now()), {
        from: 'Engineering Inventory',
        date: new Date().toLocaleString(),
        msg: `Engineering Inventory item "${inputs.name.value.trim()}" has been deleted by ${sessionStorage.getItem('username')} (BA Number: ${sessionStorage.getItem('baNumber')}).`
    }).then(() => {
